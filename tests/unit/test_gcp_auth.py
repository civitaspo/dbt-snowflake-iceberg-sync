...
from procedure.gcp_auth import (
    SnowflakeWifSubjectTokenSupplier,
    build_gcp_credentials,
    decode_jwt_expiry,
    normalize_wif_audience,
)
...
def test_normalize_wif_audience_accepts_both_spellings():
    assert normalize_wif_audience(AUDIENCE) == (AUDIENCE, "https:" + AUDIENCE)
    assert normalize_wif_audience("https:" + AUDIENCE) == (AUDIENCE, "https:" + AUDIENCE)


def test_normalize_wif_audience_rejects_other_values():
    with pytest.raises(ConfigError, match="gcp_wif_audience"):
        normalize_wif_audience("projects/000000000000/locations/global/workloadIdentityPools/p")


def test_supplier_issues_token_with_escaped_literals():
    token = make_jwt({"exp": time.time() + 900})
    session = FakeSession(token)
    supplier = SnowflakeWifSubjectTokenSupplier(session, "DB.AUTH.O'BRIEN", "https:" + AUDIENCE)
...
def test_supplier_caches_token_until_near_expiry():
    token = make_jwt({"exp": time.time() + 900})
    session = FakeSession(token)
    supplier = SnowflakeWifSubjectTokenSupplier(session, "DB.AUTH.GCP_WIF", "https:" + AUDIENCE)
...
def test_supplier_wraps_sql_failures():
    ...
    supplier = SnowflakeWifSubjectTokenSupplier(FailingSession(), "DB.AUTH.GCP_WIF", "https:" + AUDIENCE)

    with pytest.raises(SourceError, match="USAGE on the secret"):
        supplier.get_subject_token()


def test_build_wif_credentials_configures_identity_pool():
    ...
    assert isinstance(credentials._subject_token_supplier, SnowflakeWifSubjectTokenSupplier)


def test_build_wif_credentials_with_impersonation():
    session = FakeSession(make_jwt({"exp": time.time() + 900}))
    credentials = build_gcp_credentials(
        session,
        wif_deployment(gcp_service_account_impersonation="sync@example-project.iam.gserviceaccount.com"),
        secret_reader=lambda alias: pytest.fail("WIF must not read generic secrets"),
    )
...
def test_build_gcp_credentials_defaults_to_service_account_key(monkeypatch):
    ...
    deployment = DeploymentConfig.from_dict({"gcp_sa_secret_alias": "my_alias"})
