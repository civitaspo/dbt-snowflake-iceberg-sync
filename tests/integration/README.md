...
- nested BigQuery `STRUCT` field add through select export (evolves Snowflake
  `OBJECT` before COPY)
...
- nested `OBJECT` / `ARRAY(OBJECT)` evolution matrix via `evolve_schema`
  (add, reorder, widen, keep-missing+warn, deep nest, incompatible fail,
  combined add+reorder+widen)
